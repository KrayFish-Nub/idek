...
module.exports = {
    ...
    options: [
        {
            name: 'user',
            type: 'USER',
            description: 'mention the user u want to mute',
            required: true
        },
        {
            name: 'reason',
    ...
    run: async (client, interaction, args, message) => {
        ...
        if (member.id === interaction.user.id) {
            return interaction.followUp({ content: `Are you dumb? you can't mute yourself` });
        }
        ...
        const mute = new MessageEmbed()
            .setTitle('<a:tick:866661296007151626>  Mute Added').setColor(client.config.color)
            .addFields(
                { name: `Muted :`, value: `<@${member.id}>`, inline: true },
